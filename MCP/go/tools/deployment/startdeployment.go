...
func StartdeploymentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DeploymentStartRequest
		...
		url := fmt.Sprintf("%s/deployments", cfg.BaseURL)
...
func CreateStartdeploymentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_deployments",
		mcp.WithDescription("Start deployment"),
		mcp.WithString("accountName", mcp.Required(), mcp.Description("")),
		mcp.WithString("buildJobId", mcp.Description("Input parameter: Optional job id with artifacts if build contains multiple jobs.")),
		mcp.WithString("buildVersion", mcp.Required(), mcp.Description("Input parameter: Build to deploy")),
		mcp.WithString("environmentName", mcp.Required(), mcp.Description("")),
